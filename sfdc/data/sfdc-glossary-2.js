...
Activity History| The Activity History related list of a record displays all completed tasks, logged phone calls, saved interaction logs, expired events, outbound email, mass email, email added from Microsoft® Outlook®, and merged documents for the record and its associated records.
...
Advanced Function| A formula function designed for use in custom buttons, links, and s-controls. For example, the INCLUDE advanced function returns the content from an s-control snippet.
Advanced Search| A search function within Salesforce that offers search features and functionality. To access it, click Advanced Search... in the sidebar.
Add-on| An add-on is a type of quote for a current customer who wants to add a license or product through the end of the current contract.
Age| In opportunity reports, the Age of an open opportunity is the number of days since the opportunity was created. For closed opportunities, Age is the number of days between the creation date and the closed date.  In case reports, the Age of an open case is the elapsed time from creation to the present. The Age of a closed case is the elapsed time from creation to the closing time of the case. You can choose to view the Age of a case in days, hours, or minutes.
Agent| A Salesforce CRM Call Center user who handles inbound or outbound calls. An agent is usually identified by a four-digit number that serves as the agent's ID within the associated computer telephony integration (CTI) system.
Agent Console| See Salesforce Console and Console Tab.
AJAX Toolkit| A JavaScript wrapper around the API that allows you to execute any API call and access any object you have permission to view from within JavaScript code. For more information, see the AJAX Toolkit Developer's Guide.
...
Amount Without Owner Adjustment| Applies to Collaborative Forecasts. The forecast amount as seen by the forecast owner without the owner's adjustment. This is the sum of the subordinate's opportunities, including adjustments made by their manager or by the subordinate themselves, plus the rollup of the owner's own opportunities. It doesn’t include adjustments made by the forecast owner.
Ant Migration Tool| A toolkit that allows you to write an Apache Ant build script for migrating Lightning Platform components between a local file system and a Salesforce organization.
...
Reporting Snapshot Source Report| The custom report scheduled to run and load data as records into a custom object.
Reporting Snapshot Target Object| The custom object that receives the results of the source report as records.
Annuity| See Quantity Schedule and Revenue Schedule.
Anonymous Block, Apex| Apex code that does not get stored in Salesforce, but that can be compiled and executed through the use of the ExecuteAnonymousResult() API call, or the equivalent in the AJAX Toolkit.
...
Anti-Join| An anti-join is a subquery on another object in a NOT IN clause in a SOQL query. You can use anti-joins to create advanced queries. See also Semi-Join.
...
Apex-Managed Sharing| Enables developers to programmatically manipulate sharing to support their application’s behavior. Apex-managed sharing is only available for custom objects.
...
AppExchange Listing| An AppExchange listing is a description of your app or service on the AppExchange. It is your primary marketing tool for promoting your app or service to the AppExchange community.
AppExchange Publishing Organization| The AppExchange Publishing Organization (APO) is the master organization that you as a partner use to publish listings on the AppExchange. Child organizations where you develop applications can be linked to your APO, thus tying your listings together under a single provider entity to deliver a consistent message to customers.
AppExchange Security Review| The AppExchange Security Review ensures that an app is safe for customers to install. Before an app can be listed publicly on the AppExchange it must pass the AppExchange Security Review. Providers are obligated to resubmit an app for security review whenever the app is updated.
AppExchange Upgrades| Upgrading an app is the process of installing a newer version.
Application Lifecycle Management (ALM)| The process of managing an application's lifecycle, from planning, to development, to integration, to support.
Application Programming Interface (API)| The interface that a computer system, library, or application provides to allow other computer programs to request services from it and exchange data.
...
Approval Process Diagram| A visual, flowchart representation of an approval process. From a detail page of an approval process, you can open the diagram by launching the Process Visualizer.
Approval Request| An approval request is an email, Salesforce app notification, Lightning Experience notification, or Chatter post notifying the recipient that a record was submitted for approval and that his or her approval is requested.
Approval Steps| Approval steps define the chain of approval for a particular approval process. Each step determines which records can advance to that step, who to assign approval requests to, and whether to let each approver’s delegate respond to the requests. The first step specifies what to do if a record doesn’t advance to that step. Later steps specify what happens if an approver rejects the request.
Approvals in Chatter| If your organization has both Approvals and Chatter enabled, administrators can turn on Approvals in Chatter. Users then receive approval requests as posts in their Chatter feeds.
...
Article-Type Layout| An article-type layout enables administrators to create sections that organize the fields on an article, as well as choose which fields users can view and edit. One layout is available per article type. Administrators modify the layout from the article-type detail page.
...
Assigned Approver| The assigned approver is the user responsible for responding to an approval request.
...
Asynchronous Calls| A call that doesn’t return results immediately because the operation can take a long time. Calls in the Metadata API and Bulk API are asynchronous.
Autolaunched Flow| An autolaunched flow can be launched without user interaction, such as from a process or the Apex interview.start method. Autolaunched flows run in bulk and without user interaction. They can’t contain steps, screens, choices, or dynamic choices in the active or latest flow version.
Automatic Number Identification (ANI)| The number from which a caller is dialing in Salesforce CRM Call Center.
Auto Number| A custom field type that automatically adds a unique sequential number to each record. These fields are read only.
...
Batch Apex| The ability to perform long, complex operations on many records at a scheduled time using Apex.
Batch, Bulk API| A batch is a CSV, XML, or JSON representation of a set of records in the Bulk API.You can process a set of records by creating a job that contains one or more batches. The server processes each batch independently, not necessarily in the order it is received. See Job, Bulk API.
...
Beta, Managed Package| In the context of managed packages, a beta managed package is an early version of a managed package distributed to a sampling of your intended audience to test it.
...
Briefcase| The set of data that you can manage offline using Connect Offline.
...
Bulk API| The REST-based Bulk API is optimized for processing large sets of data. It allows you to query, insert, update, upsert, or delete a large number of records asynchronously by submitting a number of batches which are processed in the background by Salesforce. See also SOAP API.
Business Account| A company or organization that you want to track.
Call| Any inbound, outbound, consult, or internal voice connection via telephone.
Call Center| A group within a company or a third-party organization that handles telephone sales or service. See Salesforce CRM Call Center.
Callout, Apex| An Apex callout enables you to tightly integrate your Apex with an external service by making a call to an external Web service or sending a HTTP request from Apex code and then receiving the response.
...
Chat Window| A Web-based interface where your customers or website visitors engage in text chats with customer support agents. Customers access the Chat window by clicking a Chat button or link on your organization's website.
Chatlet| A chatlet is a mini application that runs within the Live Agent console. There are chatlets that integrate with Salesforce and Salesforce Knowledge, as well as chatlets that provide information on incoming chat requests, visitor geographical location, and browsing history.
...
Chatter Mobile| A free mobile application that lets you collaborate in Chatter from your mobile device. Use Chatter Mobile to post and comment in Chatter, and receive updates about the people, records, and files you follow and your groups.
Checkout| Checkout is the online store where you can purchase seats and other related products for Salesforce and Data.com.
Child Relationship| A relationship that has been defined on an sObject that references another sObject as the “one” side of a one-to-many relationship. For example, contacts, opportunities, and tasks have child relationships with accounts.
Class, Apex| A template or blueprint from which Apex objects are created. Classes consist of other classes, user-defined methods, variables, exception types, and static initialization code. In most cases, Apex classes are modeled on their counterparts in Java.
Client App| An app that runs outside the Salesforce user interface and uses only the Lightning Platform API or Bulk API. It typically runs on a desktop or mobile device. These apps treat the platform as a data source, using the development model of whatever tool and platform for which they are designed.
Clone| Clone is the name of a button or link that allows you to create a new item by copying the information from an existing item, for example, a contact or opportunity.
Cloud Computing| A model for software development and distribution based on the Internet. The technology infrastructure for a service, including data, is hosted on the Internet. This allows consumers to develop and use services with browsers or other thin clients instead of investing in hardware, software, or maintenance.
Cloud Flow Designer| Cloud-based application that lets administrators create a flow for use in Salesforce.
Code Coverage| A way to identify which lines of code are exercised by a set of unit tests, and which are not. This helps you identify sections of code that are completely untested and therefore at greatest risk of containing a bug or introducing a regression in the future.
Collapsible Section| Sections on detail pages that users can hide or show.
...
Comment| Comments are plain text responses to posted ideas that enable discussions about the ideas. On the Ideas tab, the Comments subtab lists the comments that have most recently been submitted. On the detail page of an idea, comments are sorted in chronological order from oldest to newest.
Comment, Chatter| A reply to a post in a Chatter feed.
...
Component, Visualforce| Something that can be added to a Visualforce page with a set of tags, for example, <apex:detail>. Visualforce includes a number of standard components, or you can create your own custom components.
Component Reference, Visualforce| A description of the standard and custom Visualforce components that are available in your organization. You can access the component library from the development footer of any Visualforce page or the Visualforce Developer's Guide.
Composite App| An app that combines native platform functionality with one or more external Web services, such as Yahoo! Maps. Composite apps allow for more flexibility and integration with other services, but may require running and managing external code. See also Client App and Native App.
...
Console Layout| Objects chosen by an administrator to display in the list view frame of the Agent console. For example, if an administrator adds cases to a console layout, then users whose profiles are assigned to that console layout can see list views of cases in the console's list view frame.
...
Consult Call| A call that results from a Salesforce CRM Call Center user initiating a conference or transfer.
...
Contract, Checkout| A contract is a written agreement defining the terms of doing business between Salesforce and your organization.
...
Controller, Visualforce| An Apex class that provides a Visualforce page with the data and business logic it needs to run. Visualforce pages can use the standard controllers that come by default with every standard or custom object, or they can use custom controllers.
...
Cooki| Client-specific data used by some Web applications to store user and session-specific information. Salesforce issues a session “cookie” only to record encrypted authentication information for the duration of a specific session.
Credit Memo| A credit memo is a pricing adjustment that credits your account.
CSV (Comma Separated Values)| A file format that enables the sharing and transportation of structured data. The import wizards, Data Loader and the Bulk API support CSV. Each line in a CSV file represents a record. A comma separates each field value in the record.
...
Custom Labels| Custom labels are custom text values that can be accessed from Apex classes, Visualforce pages, or Lightning components.
...
Custom Report Type| See Report Type.
...
Custom S-Control| Note S-controls have been superseded by Visualforce pages. After March 2010 organizations that have never created s-controls, as well as new organizations, won't be allowed to create them. Existing s-controls will remain unaffected, and can still be edited.  Custom Web content for use in custom links. Custom s-controls can contain any type of content that you can display in a browser, for example a Java applet, an Active-X control, an Excel file, or a custom HTML Web form.
Custom View| A display feature that lets you see a specific set of records for a particular object.
Customer Relationship Management (CRM)| A business strategy for developing and improving relations between companies and their customers.
Customers, Chatter| Users outside of your company's email domains. They have very limited Chatter access and can only see groups they're invited to and interact with members of those groups.
...
Dashboard Filters| Filters on a dashboard allow you to choose different views of data.
...
Data Loader| A Lightning Platform tool used to import and export data from your Salesforce organization.
Data Manipulation Language (DML)| An Apex method or operation that inserts, updates, or deletes records.
Data State| The structure of data in an object at a particular point in time.
Database| An organized collection of information. The underlying architecture of the Lightning Platform includes a database where your data is stored.
Database Table| A list of information, presented with rows and columns, about the person, thing, or concept you want to track. See also Object.
Database.com| Database.com is an enterprise cloud database from Salesforce.
Date Literal| A keyword in a SOQL or SOSL query that represents a relative range of time such as last month or next year.
...
Decimal Places| Parameter for number, currency, and percent custom fields that indicates the total number of digits you can enter to the right of a decimal point, for example, 4.98 for an entry of 2. Note that the system rounds the decimal numbers you enter, if necessary. For example, if you enter 4.986 in a field with Decimal Places of 2, the number rounds to 4.99. Salesforce uses the round half-up rounding algorithm. Half-way values are always rounded up. For example, 1.45 is rounded to 1.5. –1.45 is rounded to –1.5.
Delegated Administration| A security model in which a group of non-administrator users perform administrative tasks.
Delegated Authentication| A security process where an external authority is used to authenticate Lightning Platform users.
Delegated Approver| A delegated approver is a user appointed by an assigned approver as an alternate for approval requests.
...
Salesforce Developers| The Salesforce Developers website at developer.salesforce.com provides a full range of resources for platform developers, including sample code, toolkits, an online developer community, and the ability to obtain limited Lightning Platform environments.
...
Development as a Service (DaaS)| An application development model where all development is on the Web. This means that source code, compilation, and development environments are not on local machines, but are Web-based services.
Development Environment| A Salesforce organization where you can make configuration changes that will not affect users on the production organization. There are two kinds of development environments, sandboxes and Developer Edition organizations.
Dial-Tone Multi-Frequency (DTMF)| The system that informs a switch of the number that is being pressed by a caller in Salesforce CRM Call Center.
Dialed Number Identification Service (DNIS)| The number a caller dialed in Salesforce CRM Call Center.
Direct Inward Dial (DID)| A phone number that allows a Salesforce CRM Call Center user to be reached directly from outside the call center. DIDs can be used in place of extensions, which require a caller to first dial the main private branch exchange number and then enter an extension.
Directory Number (DN)| Any internal number that is configured on a public branch exchange. You can define additional directory numbers through the Salesforce CRM Call Center setup within Salesforce.
...
Dynamic Dashboard| A dynamic dashboard runs using the security settings of the user viewing the dashboard. Each user sees the dashboard according to his or her own access level. This approach helps administrators share one common set of dashboard components to users with different levels of access.
Dynamic Visualforce Binding| A way of writing generic Visualforce pages that display information about records without necessarily knowing which fields to show. In other words, fields on the page are determined at runtime, rather than compile time.
Email Alert| Email alerts are actions that send emails, using a specified email template, to specified recipients.
Email Approval Response| Email approval response lets users respond to approval requests by replying to an email notification.
Email Notification| An email that's sent when activities happen in Chatter. You can configure the activities that trigger email notifications.
...
Encryption Key| Encryption keys are used by an encryption algorithm to convert plaintext values into ciphertext and back. Platform Encryption uses the AES algorithm and 256-bit symmetric keys with a random IV (Initialization Vector) to encrypt and decrypt data stored at rest on the Salesforce platform.
Encryption Key Management| Refers to all aspects of key management, such as key creation, processes, and storage. Tenant secret management is performed by administrators or users who have the “Manage Encryption Keys” permission, enabled from Setup by clicking Security Controls | Platform Encryption.
Enterprise Application| An application that is designed to support functionality for an organization as a whole, rather than solving a specific problem.
...
Entity Relationship Diagram (ERD)| A data modeling tool that helps you organize your data into entities (or objects, as they are called in the Lightning Platform) and define the relationships between them. ERD diagrams for key Salesforce objects are published in the SOAP API Developer’s Guide.
Enumeration Field| An enumeration is the WSDL equivalent of a picklist field. The valid values of the field are restricted to a strict set of possible values, all having the same data type.
Event| An event is an activity that has a scheduled time. For example, a meeting, or a scheduled phone call.
Event Series| A series of events that recur with a regular pattern. For example, a weekly meeting with a customer defined as an event series.
Event Sink, CTI| An object in a computer telephony integration (CTI) connector that receives CTI system events and routes them to other methods for processing.
...
Home Organization
The organization used for retrieving components to a local file system. If using the Force.com IDE, the home organization is the organization used to create a project. If using the Ant Migration Tool, the home organization is the server you specify in build.properties when you retrieve components.
Home Tab
...
Live Agent Console
The Live Agent console is a dedicated console used by customer service agents to manage Live Agent chats. Agents can answer chat requests, conduct live chats, and interact with Salesforce records and Salesforce Knowledge articles.
Local Name
...
Manifest File
The project manifest file (package.xml) lists the XML components to retrieve or deploy when working with the Metadata API, or clients built on top of the Metadata API, such as the Force.com IDE or the Ant Migration Tool.
Manual Sharing
...
Quota
The sales goal assigned to a user on a monthly and quarterly basis. (If you use Collaborative Forecasts, it’s the sales goal assigned to a user on a monthly basis.) A manager’s quota should equal the amount she and her team are expected to generate together.
Quote
...
An API that uses browsers as clients to display pages as tabs in a Salesforce console; it provides developers with programmatic access to the console so that administrators can extend it to meet your business needs.
...
Setup
A menu where administrators can customize and define organization settings and Lightning Platform apps. Depending on your organization’s user interface settings, Setup may be a link in the user interface header or in the drop-down list under your name.
Share Group
...
A query language that allows you to construct simple but powerful query strings and to specify the criteria that should be used to select data from the Lightning Platform database.
SOSL (Salesforce Object Search Language)
A query language that allows you to perform text-based searches using the Lightning Platform API.
Source Report
A custom report scheduled to run and load data as records into a target object for an reporting snapshot.
Special Terms
Any terms that you have specifically agreed to and want to track in your contract.
Stage Duration
...
The subscriber org uses the croos-org adapter for Salesforce Connect to access data that's stored in another (provider) org. The subscriber org contains the external data source definition and the external objects that map to the provider org’s objects.
Summary Field
...
Top All Time Ideas
On the Top All-Time subtab, ideas are sorted from most number of points to fewest. This allows you to see the most popular ideas in the history of your Ideas zone.
Topics
...
Truncate
Truncating a custom object lets you remove all of the object’s records, while keeping the object and its metadata.
U
Unfollow
...
Vote, Idea
In an ideas community, a vote means you have either promoted or demoted an idea. After you vote on an idea, your nickname displays at the bottom of the idea's detail page to track that your vote was made. You can use your Recent Activity page to see a list of all the ideas you have voted on.
Vote, Reply
